var mobileGraph = function(element, readings, options){
	var bgReadings = readings.cgm;
	var carbReadings = readings.carb;
	var bolusReadings = readings.bolus;
	var correctedMeals = readings.correctedMeals;
	...
	var dot = function(readings, options) {
		...
		drawYAxisLabels([40,80,120,200,300], yScale, options.size === 'small' ? 'svg-label-axis-small' : 'svg-label-axis');
	...
	var labels = function(inputLabel) {
		var labels = [
		  {text:'BLOOD GLUCOSE', css: options.size === 'small' ? 'svg-label-small' : 'svg-label' , x: 50, y: 34}
	...
	if(state.glucagon) {
		...
		if (state.carb.value == 2) {
			...
			drawWe.carbs(carbReadings, rOptions);

			rOptions.paddingTop = paddingTop + 7 + rOptions.height;
			drawWe.bolus(bolusReadings, rOptions);

			rOptions.paddingTop = paddingTop + 7 + rOptions.height*2;
			drawWe.glucagon(carbReadings, rOptions);
		...
		if (state.carb.value == 2) {
			...
			rOptions.paddingTop = paddingTop + rOptions.height;
			drawWe.bolus(bolusReadings, rOptions);
